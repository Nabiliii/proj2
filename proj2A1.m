...
function  [Xe,P ] = Do_update_OOI (Xe,P, OOI, landmark,Lidar1Cfg, ranges,colors,ym )
    ...
    R = 0.25^2;% [ [ 0.25^2   ,        0     ]     %assuming the var is 25, not the SD
          %[   0    , (3*pi/180)^2 ] ];  % OOIs bearing observations: standard deviation is not used 
...
%      [ (yk - y)/((xk - x)^2 + (yk - y)^2)         ,  -(xk - x)/((xk - x)^2 + (yk - y)^2)        ,-1]
%    ];
